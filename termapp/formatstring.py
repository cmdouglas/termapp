...
FORMAT_BLINK = '\x1b[5m'
FORMAT_INVERT = '\x1b[7m'
FORMAT_NORMAL = '\x1b(B\x1b[m'


color_pattern = '(?P<color>' + '|'.join(colors.colors.keys()) + ')?'
bgcolor_pattern = '(?P<bgcolor>' + '|'.join(colors.bg_colors.keys()) + ')?'
format_pattern = '(?P<format>[ubi]+)'
...
class Format:
    __slots__ = ['color', 'bgcolor', 'underline', 'blink', 'invert']

    def __init__(self, color=None, bgcolor=None, underline=False, blink=False, invert=False):
        self.color = color
    ...
    def apply_to(self, s: str):
        ...
        if self.underline:
            formats.append(FORMAT_UNDERLINE)

        if self.blink:
            formats.append(FORMAT_BLINK)

        if self.invert:
            formats.append(FORMAT_INVERT)

        if formats:
            formats.append(s)
            formats.append(FORMAT_NORMAL)
            s = "".join(formats)
...
class FormatStringChunk:
    __slots__ = ['s', 'format', '_rendered_string']
...
class FormatString:
    __slots__ = ['condensed', 'chunks', '_rendered']
    ...
    def __str__(self):
        if self._rendered is None:
            self.condense()
            self._rendered = "".join([str(chunk) for chunk in self.chunks])
    ...
    def parse(self, string: str):
        ...
        for match in re.finditer(match_pattern, string):
